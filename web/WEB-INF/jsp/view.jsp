...
<c:forEach var="sectionEntry" items="${resume.sections}">
    ...
    <c:choose>
        <c:when test="${type=='OBJECTIVE'}">
            <div class="position"><%=((SimpleTextSection) section).getContent()%>
            </div>
        </c:when>
        <c:when test="${type=='PERSONAL'}">
            <div class="qualities"><%=((SimpleTextSection) section).getContent()%>
            </div>
        </c:when>
        ...
        <c:when test="${type=='EXPERIENCE' || type=='EDUCATION'}">
            <c:forEach var="org" items="<%=((OrganizationsSection) section).getOrganizations()%>">
                ...
                <c:forEach var="position" items="${org.positions}">
                    ...
                    <div class="period"><%=HtmlHelper.formatDates(position)%>
                    </div>
